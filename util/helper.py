...
def load_models(app_model_path, app_model_path_track, mvmodel_path, bimodel_path, bimodel_path_track, ind):
    ...
    return appmodel, appmodel_track, mvmodel, combinemodel, combinemodel_track

# Initialize parameters
def initialize_params(a, b):
    return {
        'downrightx': 350,
        'upleftx': 0,
        'downrighty': 780,
        'uplefty': 510,
        'fileName': 'supervised_SVM_1',
        'debug': 1,
        'qualityini': 0.005, 
...
def is_close(coord1, coord2, threshold=50):
    """Check if two coordinates are close within a given threshold."""
...
def combine_bounding_boxes(box1, box2):
    ...
    y1 = min(box1[1] - box1[2]//2, box2[1] - box2[3]//2)
    x2 = max(box1[0] + box1[2]//2, box2[0] + box2[2]//2)
    y2 = max(box1[1] + box1[3]//2, box2[1] + box2[3]//2)
    return (x1, y1, x2, y2) ### (minx, miny, maxx, maxy)  

def update_tracking_id(id1, id2, tracker, common_id):
    # Get the common tracking ID
    # common_id = min(id1, id2)

    # Update the tracker
    if id1 == tracker[0] or id2 == tracker[1] or (id1, id2) == tracker:
        tracker = (id1, id2)
        return common_id, tracker
    else:
        tracker = (id1, id2)
        return common_id + 1, tracker
